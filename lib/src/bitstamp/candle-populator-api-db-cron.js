...
async function getCandle(timestamp, retryCount = 0) {
  ...
  if (!bitstampLastCandle || !(+bitstampLastCandle.volume)) {
    await wait(1000);
    return retryCount < 20 ? getCandle(timestamp, retryCount + 1) : null;
  };
  const candle = {};
