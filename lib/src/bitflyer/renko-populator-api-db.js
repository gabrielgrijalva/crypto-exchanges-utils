...
async function syncRenkoCandles() {
  ...
  while (beforeId <= initialTradeId && prevBeforeId !== beforeId) {
    ...
    for (let i = 0; trades[i]; i += 1) {
      ...
      const closeCurrentCandleMinTrades = candleInfo.numTrades
        >= settings.CANDLE_MIN_TRADES;
      if (closeCurrentCandlePriceRef && closeCurrentCandleMinTrades) {
...
function onMessageFunction(message) {
  ...
  messageParsed.forEach(trade => {
    ...
    const closeCurrentCandleMinTrades = currentCandleInfo.numTrades
      >= settings.CANDLE_MIN_TRADES;
    if (closeCurrentCandlePriceRef && closeCurrentCandleMinTrades) {
