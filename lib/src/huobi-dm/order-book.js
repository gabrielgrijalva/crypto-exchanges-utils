...
module.exports = function HuobiDMOrderBook(symbol) {
  ...
  const huobiDMOrderBook = {
    ...
    disconnect: function () {
      ...
      clearInterval(this.lastUpdateIntervalId);
    },
  };
  ...
  function onClose() {
    ...
    if (huobiDMOrderBook.status !== 'disconnected') {
  ...
  huobiDMOrderBook.lastUpdateIntervalId = setInterval(() => {
    if (huobiDMOrderBook.status === 'connected') {
      const currentTimestamp = Date.now();
      const lastUpdateTimestamp = huobiDMOrderBook
        .lastUpdateTimestamp;

      if (lastUpdateTimestamp && (currentTimestamp - lastUpdateTimestamp)
        > 5000) {
